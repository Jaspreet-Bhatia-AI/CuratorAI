  ...
  return (
    <div className="pt-24 px-6 max-w-7xl mx-auto pb-32 min-h-screen">
      <div className="flex justify-between items-end mb-10">
        <div>
          <h1 className="text-4xl font-bold text-slate-900 dark:text-white mb-2 tracking-tight">Your Library</h1>
          <p className="text-slate-500 dark:text-slate-400">Your offline collection, stored directly in this app.</p>
        </div>
        <div className="flex gap-4">
          <button onClick={loadDemoSong} className="bg-slate-100 dark:bg-slate-800 hover:bg-slate-700 text-slate-900 dark:text-white px-4 py-2 rounded-lg text-sm transition-colors border border-slate-200 dark:border-slate-700">
            + Add Demo Track
          </button>
          <button className="bg-gradient-to-r from-google-purple to-blue-500 text-slate-900 dark:text-white px-4 py-2 rounded-lg text-sm font-semibold hover:opacity-90 transition-opacity flex items-center gap-2">
      ...
      {songs.length === 0 ? (
        <div className="text-center py-20 bg-white dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800/50">
          ...
          <h3 className="text-xl font-semibold text-slate-900 dark:text-white mb-2">Your library is empty</h3>
          <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto">When you curate playlists, you can save songs here for offline listening without filling up your phone's download folder.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {songs.map((song, index) => (
            <motion.div
              ...
              className={`group flex items-center p-3 rounded-xl cursor-pointer transition-colors ${currentSong?.id === song.id ? 'bg-black/10 dark:bg-white/10 border border-white/20' : 'hover:bg-slate-100 dark:bg-slate-800/50 border border-transparent'}`}
            >
              <div className="w-12 h-12 rounded-lg bg-slate-100 dark:bg-slate-800 overflow-hidden relative flex-shrink-0">
                ...
                <div className={`absolute inset-0 bg-black/40 flex items-center justify-center ${currentSong?.id === song.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} transition-opacity`}>
                  {currentSong?.id === song.id && isPlaying ? (
                    <svg className="w-5 h-5 text-slate-900 dark:text-white" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zM7 8a1 1 0 012 0v4a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v4a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd" /></svg>
                  ) : (
                    <svg className="w-5 h-5 text-slate-900 dark:text-white" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd" /></svg>
                  )}
                </div>
              </div>
              <div className="ml-4 flex-1">
                <h4 className={`font-medium ${currentSong?.id === song.id ? 'text-google-purple' : 'text-slate-900 dark:text-white'}`}>{song.title}</h4>
      ...
      <AnimatePresence>
        {currentSong && (
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 z-40 bg-white dark:bg-slate-900/95 backdrop-blur-xl border-t border-black/10 dark:border-white/10 px-6 py-4"
          >
            <div className="max-w-7xl mx-auto flex items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-4 w-1/4">
                ...
                <div className="overflow-hidden">
                  <h4 className="text-slate-900 dark:text-white font-medium truncate">{currentSong.title}</h4>
              ...
              <div className="flex flex-col items-center flex-1 max-w-2xl">
                <div className="flex items-center gap-6 mb-2">
                  <button className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors">
                  ...
                  <button onClick={playNext} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 5l7 7-7 7M5 5l7 7-7 7"></path></svg>
                  </button>
                </div>
                
                {/* Progress Bar */}
                <div className="w-full flex items-center gap-3 text-xs text-slate-500 dark:text-slate-400 font-mono">
                  <span>{formatTime(audioRef.current.currentTime)}</span>
                  <div className="h-1.5 flex-1 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden cursor-pointer">
                    <div 
