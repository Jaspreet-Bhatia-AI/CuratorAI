...
export default function Hero({ onSearch, hasSearched, isLoading }) {
      ...
      >
        ...
        <div className="relative flex items-center bg-google-surface/60 border border-white/10 backdrop-blur-xl p-2 rounded-[2rem]">
          <input 
            ...
            disabled={isLoading}
            placeholder={isLoading ? "AI is architecting your curriculum..." : "What do you want to learn today?"}
            className="w-full bg-transparent border-none outline-none text-white px-6 py-4 placeholder-gray-500 text-lg disabled:opacity-50"
          />
          <button 
            type="submit"
            disabled={isLoading}
            className={`bg-white/10 hover:bg-white/20 text-white p-4 rounded-full backdrop-blur-md transition-all duration-300 ${isLoading ? 'animate-pulse' : ''}`}
          >
            {isLoading ? (
